...
def search_graph(graph):
    num_node, edge_set = len(graph.x), set()
...
class MoleculeDataset_GPTGNN(InMemoryDataset):
    def __init__(self, molecule_dataset, transform=None, pre_transform=None):
        ...
        super(MoleculeDataset_GPTGNN, self).__init__(
            self.root, transform=transform, pre_transform=pre_transform
        )
    ...
    def process(self):
        num_molecule, data_list = len(self.molecule_dataset), list()
        for i in tqdm(range(num_molecule)):
            ...
            for idx in range(num_node - 1):
                ...
                # Take the subgraph and predict the next node (atom type only)
                sub_graph = Data(
                    x=graph.x[sub_node_list],
                    edge_index=edge_index,
                    edge_attr=edge_attr,
                    next_x=graph.x[next_node, :1],
                )
